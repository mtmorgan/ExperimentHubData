...
```
aws --profile AnnotationContributor s3 cp test_file.txt
s3://annotation-contributor/test_file.txt --acl public-read
...
* Location_Prefix: 'character(1)'. Do not include this field if data are stored
  ...
  url.
* RDataPath: 'character()'.This field should be the remainder of the path to
  ...
  resource file name. For strongly associated files, like a bam file and its
  index file, the two files should be separates with a colon. This will link a
  single hub id with the multiple files.
