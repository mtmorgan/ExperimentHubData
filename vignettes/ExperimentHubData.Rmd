...
# Adding new resources to an existing package

Adding new resources to an existing package requires the following steps:

* upload new data to S3 bucket

* update make-metadata.R with new resource metadata

* re-generate metadata.csv
...
* upload new file to S3 bucket to which you have permissions; the new
  file must have the same name as the original
